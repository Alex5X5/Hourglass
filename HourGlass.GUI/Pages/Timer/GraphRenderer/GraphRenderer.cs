...
public abstract class GraphRenderer : Panel {
    ...
    public abstract int TASK_GRAPH_COUNT { get; }
	...
	protected async override void OnClick(EventArgs e) {
		...
		if (!taskClicked) {
			if (WindowMode == TimerWindowMode.Week) {
				...
				_parent.ChangeSelectedDay(DateTimeService.GetMondayOfWeekAtDate(_parent.SelectedWeek).AddDays(offset));
				_parent.ChangeGraphRendererMode(TimerWindowMode.Day);
				...
				_parent.ChangeSelectedDay(DateTimeService.GetMondayOfWeekAtDate(DateTimeService.GetFirstDayOfMonthAtDate(DateTime.Now)));
				_parent.ChangeGraphRendererMode(TimerWindowMode.Week);
			}
		}
	}
}
